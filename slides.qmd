...
3. Install ellmer: `install.packages("ellmer")`
...
::: {.notes}
- Hello world of ellmer and chatlas
- Two lines of code in both languages
- Create chat object - R uses chat() with provider string, Python uses provider-specific classes
- Call chat method and get response
...
chat$chat("What's the weather like in Foster City?")
```
:::
...
Tools are extra capabilities that you give to the LLM, like the ability to search the BBSW 2025 schedule or check the weather.
...
```r
get_weather <- tool(
  \(lat, lon) point_forecast(lat, lon),
  name = "get_weather",
...
def get_weather(lat: float, lon: float):
  ...
  return point_forecast(lat, lon)

# Type hints + docstring = metadata!
...
::: {style="--code-font-size: 0.75em"}
```{.r}
chat$chat("What's the weather in Foster City?")
```
:::
...
```{.markdown code-line-numbers="false" style="border: none !important; box-shadow: none !important;" code-line-numbers="1|2-4|5-10"}
◯ [tool call] get_weather(lat = 37.5585, lon = -122.2711)
● #> [{"time":"2025-11-06 10:00:00 PST","temp":58,"dewpoint":12.2222,...
Here's the current weather forecast for Foster City, California:

**Current Conditions (as of 10 PM PST):**
- Temperature: 58°F
- Conditions: Partly Cloudy
- Humidity: 87%
- Wind: 6 mph from the Northwest
- Very low chance of rain (4%)
```
:::


::: notes
Let's look at how to create and use a tool.

First, we define a tool using the `tool()` function. It wraps an R function and adds metadata - a name, description, and argument definitions. This metadata helps the LLM understand when and how to use the tool.

The tool itself is just an R function that calls weathR to get weather data.

Then we register the tool with our chat object using `register_tool()`.

Now when we ask about what to wear, the LLM can call the weather tool to get real-time data!
:::
...
* They incorporate your this information into queries sent to the LLM
...
* **Provider:** The company that hosts the model (Anthropic, OpenAI, Google, etc. Could also be something like HuggingFace)
